...
def determine_frontier_path(state: SimulationState) -> None:
    ...
    if state.frontier_goal == state.goal["cell"]:
        ...
        # Robustness check: If no frontiers are detected, return directly to avoid program crashes.
        if not frontiers:
            log.warning("No frontiers detected. Robot might be trapped or exploration is complete.")
            return

        # Get key information needed for decision making:
        ...
        # Calculate the unit vector of the robot's heading for subsequent projection and alignment calculations.
        heading_vec = (math.cos(robot_heading), math.sin(robot_heading))

        # Definition of helper functions:
        # These functions are used to evaluate the quality of each frontier.

        def get_heading_projection(cell: Cell) -> float:
            """Calculate the projection distance of the frontier in the robot's forward direction. A larger value means it is more directly in front."""
            center = cell_center(cell, cell_size)
            dx = center[0] - robot_pose["x"]
            dy = center[1] - robot_pose["y"]
            # Vector dot product: (dx, dy) · (heading_vec_x, heading_vec_y)
            return dx * heading_vec[0] + dy * heading_vec[1]

        def get_heading_alignment(cell: Cell) -> float:
            """Calculate the alignment between the frontier direction and the robot's heading (cosine of the angle). A larger value means the directions are more aligned."""
            center = cell_center(cell, cell_size)
            dx = center[0] - robot_pose["x"]
            dy = center[1] - robot_pose["y"]
            ...
            if length < 1e-9:
                return 1.0  # If the frontier is at the robot's current position, consider it perfectly aligned.
            # (dx, dy) · heading_vec / |(dx, dy)|
            return (dx * heading_vec[0] + dy * heading_vec[1]) / length

        def get_dist_to_goal(cell: Cell) -> float:
            """Calculate the Euclidean distance from the frontier to the final goal."""
            center = cell_center(cell, cell_size)
            return math.hypot(center[0] - goal_center[0], center[1] - goal_center[1])

        # 1. Prioritize frontiers in the "forward direction".
        # "Forward" is defined as frontiers within the 90-degree cone in front of the robot (heading_alignment > 0).
        forward_frontiers = [cell for cell in frontiers if get_heading_alignment(cell) > 0]
        
        # If there are "forward" frontiers, select from them; otherwise, select from all frontiers.
        # This is to ensure that the robot can find a way out even if it is facing a wall.
        candidate_pool = forward_frontiers if forward_frontiers else frontiers

        # 2. Use a multi-criteria scoring system to select the best frontier.
        # The max function compares elements of the tuple sequentially to find the optimal solution.
        best_frontier_cell = max(
            candidate_pool,
            key=lambda cell: (
                # Priority 1: Projection distance (the larger the better), encourages the robot to move in a straight line.
                get_heading_projection(cell),
                # Priority 2: Heading alignment (the larger the better), to reduce in-place turning.
                get_heading_alignment(cell),
                # Priority 3: Distance to the final goal (the smaller the better, hence the negative sign).
                -get_dist_to_goal(cell),
                # Priority 4: Path distance from the robot (the smaller the better, hence the negative sign).
                -distances[cell],
            ),
        )
