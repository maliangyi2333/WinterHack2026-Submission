...
DEFAULTS: Dict[str, Dict] = {
    ...
    "random_maze": {
        ...
        "cell_size_m": 0.45,
        "random_wall_count": 9,
        "random_seed": None,  #None, 6, 243, 463
...
def determine_frontier_path(state: SimulationState) -> None:
    ...
    if state.frontier_goal == state.goal["cell"]:
        ...
        def goal_distance(cell: Cell) -> float:
            """
            Calculate the Euclidean distance from a given frontier cell to the ultimate goal cell.
            Args:
                cell (Cell): The frontier cell to calculate distance for.
            Returns:
                float: Distance from the frontier cell to the goal cell.
            """
            center = cell_center(cell, cell_size)
            return math.hypot(center[0] - goal_center[0], center[1] - goal_center[1])

        def robot_distance(cell: Cell) -> float:
            """
            Calculate the Euclidean distance from a given frontier cell to the current robot position.
            Args:
                cell (Cell): The frontier cell to calculate distance for.
            Returns:
                float: Distance from the frontier cell to the robot position.
            """
            center = cell_center(cell, cell_size)
            return math.hypot(center[0] - state.pose["x"], center[1] - state.pose["y"])

        def heading_alignment(cell: Cell) -> float:
            """
            Calculate the alignment (cosine similarity) between a given frontier cell and the robot's current heading.
            Values closer to 1 indicate better alignment, values closer to -1 indicate opposite direction.
            Args:
                cell (Cell): The frontier cell to calculate alignment for.
            Returns:
                float: Heading alignment score.
            """
            center = cell_center(cell, cell_size)
        ...
        def is_goal_visible(goal_cell: Cell) -> bool:
            """
            Check if the goal cell is reachable via a clear path in the occupancy grid map.
            Uses stricter criteria to avoid premature goal locking in U-turns.
            Args:
                goal_cell (Cell): The ultimate goal cell.
            Returns:
            ...
            # Check if goal cell itself is free
            p = prob[iy, ix]
            if p > free_thresh:
                return False
            
            # Additional check: verify path to goal is mostly clear
            robot_x, robot_y = state.pose["x"], state.pose["y"]
            rx_idx = int((robot_x - minx) / res)
            ry_idx = int((robot_y - miny) / res)
            
            # Simple line-of-sight check
            steps = max(abs(ix - rx_idx), abs(iy - ry_idx))
            if steps == 0:
                return True
            ...
            for i in range(steps + 1):
                t = i / max(steps, 1)
                check_x = int(rx_idx + t * (ix - rx_idx))
                check_y = int(ry_idx + t * (iy - ry_idx))
                
                if 0 <= check_x < width and 0 <= check_y < height:
                    total_count += 1
                    if prob[check_y, check_x] <= free_thresh:
                        free_count += 1
            
            # Require at least 70% of path to be free
            return total_count > 0 and (free_count / total_count) >= 0.7

        def get_cell_status(cell: Cell) -> str:
            """
            Get the status of a given cell in the occupancy grid map (OGM) ("occupied", "free", "unknown").
            Args:
                cell (Cell): The cell to query.
            Returns:
                str: The cell's status.
            """
            ogm = state.ogm
            grid = ogm["grid"]
            prob = 1 / (1 + np.exp(-grid))
        ...
        def openness_score(cell: Cell) -> float:
            """
            Calculate the openness around a frontier to avoid selecting frontiers leading to dead-ends.
            Evaluates by checking the number of free and unknown cells within a certain range around the frontier.
            Args:
                cell (Cell): The frontier cell to evaluate.
            Returns:
                float: Openness score, higher values indicate more open space.
            """
            ...
            openness = (free_count * 1.0 + unknown_count * 0.5) / total_checked
            return openness

        def update_frontier_tracker(current_goal: Cell) -> bool:
            """
            Track the robot's exploration attempts for a specific frontier.
            Used to detect if the robot is stuck near a frontier or making slow progress, and blacklist it accordingly.
            Args:
                current_goal (Cell): The frontier cell the robot is currently heading towards.
            Returns:
                bool: True if the robot should continue towards this frontier, False otherwise.
            """
            if not hasattr(state, 'frontier_tracker'):
                state.frontier_tracker = {}
            
            if not hasattr(state, 'frontier_blacklist'):
                state.frontier_blacklist = set()
            
            current_pos = (state.pose["x"], state.pose["y"])
            
            if current_goal not in state.frontier_tracker:
                state.frontier_tracker[current_goal] = {
                    'total_steps': 0,
                    'positions': [current_pos],
                    'min_distance': robot_distance(current_goal),
                    'initial_distance': robot_distance(current_goal),
                    'last_check_step': state.step
                }
                return True
            
            tracker = state.frontier_tracker[current_goal]
            ...
            current_dist = robot_distance(current_goal)
            if current_dist < tracker['min_distance']:
                tracker['min_distance'] = current_dist
            
            if tracker['total_steps'] > 30:
                state.frontier_blacklist.add(current_goal)
                return False
            
            if len(tracker['positions']) >= 15:
                recent_pos = tracker['positions'][-15:]
                x_coords = [p[0] for p in recent_pos]
                y_coords = [p[1] for p in recent_pos]
                
                x_var = sum((x - sum(x_coords)/len(x_coords))**2 for x in x_coords) / len(x_coords)
                y_var = sum((y - sum(y_coords)/len(y_coords))**2 for y in y_coords) / len(y_coords)
                
                total_var = (x_var + y_var) ** 0.5
                
                if total_var < 0.08 and tracker['total_steps'] > 12:
                    state.frontier_blacklist.add(current_goal)
                    return False
            
            if current_dist < cell_size * 0.5 and tracker['total_steps'] > 10:
                improvement = tracker['initial_distance'] - current_dist
                if improvement < 0.1:
                    state.frontier_blacklist.add(current_goal)
                    return False
        ...
        def is_in_blacklist_area(cell: Cell) -> bool:
            """
            Check if a given cell is near a blacklisted area.
            Args:
                cell (Cell): The cell to check.
            Returns:
                bool: True if near a blacklisted area, False otherwise.
            """
            if not hasattr(state, 'frontier_blacklist'):
                return False
            
            BLACKLIST_RADIUS = 1
            for blacklisted in state.frontier_blacklist:
                if abs(cell[0] - blacklisted[0]) <= BLACKLIST_RADIUS and abs(cell[1] - blacklisted[1]) <= BLACKLIST_RADIUS:
                    return True
            
            return False

        def is_current_path_valid() -> bool:
            """
            Check if the current path is still valid and making progress.
            Returns:
                bool: True if current path should be maintained, False if needs replanning.
            """
            if not state.path or len(state.path) < 2:
                return False
            
            if not hasattr(state, 'frontier_goal') or state.frontier_goal is None:
                return False
            
            # Check if we're making progress towards current frontier
            if hasattr(state, 'last_frontier_distance'):
                current_dist = robot_distance(state.frontier_goal)
                # If distance increased significantly, path might be invalid
                if current_dist > state.last_frontier_distance + cell_size * 0.5:
                    return False
            
            # Check if current frontier is still in the frontier list
            if state.frontier_goal not in frontiers:
                return False
            
            return True

        def exploration_value(cell: Cell) -> float:
            """
            Comprehensive scoring function for frontier selection.
            Args:
                cell (Cell): The frontier cell to evaluate.
            Returns:
                float: Comprehensive score, higher scores indicate more valuable frontiers.
            """
            if is_in_blacklist_area(cell):
                return -1000.0
            
            r_dist = robot_distance(cell)
            g_dist = goal_distance(cell)
            r_dist = max(r_dist, 1e-6)
            g_dist = max(g_dist, 1e-6)
            
            alignment = heading_alignment(cell)
            openness = openness_score(cell)
            
            bfs_dist = distances[cell]
            max_bfs = max(distances.values()) if distances else 1
            normalized_bfs = bfs_dist / max(max_bfs, 1)
            
            attempt_penalty = 0.0
            if hasattr(state, 'frontier_tracker') and cell in state.frontier_tracker:
                tracker = state.frontier_tracker[cell]
                if tracker['total_steps'] > 5: 
                    attempt_penalty = -2.0
            
            # Bonus for current frontier to maintain stability
            stability_bonus = 0.0
            if hasattr(state, 'frontier_goal') and state.frontier_goal == cell:
                # Only give bonus if we're making progress
                if hasattr(state, 'last_frontier_distance'):
                    current_dist = robot_distance(cell)
                    if current_dist <= state.last_frontier_distance:
                        stability_bonus = 2.0  # Significant bonus to avoid switching
        ...
        # Check if we should maintain current path
        maintain_current = False
        if hasattr(state, 'frontier_goal') and state.frontier_goal is not None:
            if is_current_path_valid():
                current_dist = robot_distance(state.frontier_goal)
                # If we're close to current frontier or making good progress, maintain it
                if current_dist < cell_size * 2.0 or (hasattr(state, 'last_frontier_distance') and 
                                                    current_dist < state.last_frontier_distance):
                    maintain_current = True
                    # Update distance tracking
                    state.last_frontier_distance = current_dist

        if maintain_current and state.frontier_goal in frontiers:
            # Keep current frontier to avoid oscillation
            best_frontier_cell = state.frontier_goal
        else:
            # Need to select new frontier
            if is_goal_visible(goal_cell):
                best_frontier_cell = goal_cell
                # Clear tracking when switching to goal
                if hasattr(state, 'last_frontier_distance'):
                    del state.last_frontier_distance
            else:
                if not frontiers:
                ...
                current_frontier = state.frontier_goal if hasattr(state, 'frontier_goal') else None
                
                if current_frontier and current_frontier in frontiers:
                    should_continue = update_frontier_tracker(current_frontier)
                    if not should_continue:
                        current_frontier = None
                
                MIN_FRONTIER_DIST = cell_size * 0.25
                
                valid_frontiers = [
                    cell for cell in frontiers 
                    if robot_distance(cell) > MIN_FRONTIER_DIST and not is_in_blacklist_area(cell)
                ]
                
                if not valid_frontiers:
                    state.frontier_blacklist.clear()
                    if hasattr(state, 'frontier_tracker'):
                        state.frontier_tracker.clear()
                    valid_frontiers = [cell for cell in frontiers if robot_distance(cell) > MIN_FRONTIER_DIST]
                
                if not valid_frontiers:
                    valid_frontiers = frontiers
                
                best_frontier_cell = max(valid_frontiers, key=exploration_value)
                
                # Initialize distance tracking for new frontier
                state.last_frontier_distance = robot_distance(best_frontier_cell)
                
                if best_frontier_cell != current_frontier:                
                    sorted_frontiers = sorted(valid_frontiers, key=exploration_value, reverse=True)[:5]
                    for i, f in enumerate(sorted_frontiers):
                        score = exploration_value(f)
                        dist = robot_distance(f)
                        g_dist = goal_distance(f)
                        op = openness_score(f)
                        bfs = distances[f]
                        print(f"   {i+1}. {f}: Score={score:.1f}, Robot distance={dist:.2f}, Goal distance={g_dist:.2f}, BFS distance={bfs}, Openness score={op:.2f}")
